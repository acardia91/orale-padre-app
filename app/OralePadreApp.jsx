...
import { loadAllData, savePromotions, saveStockAlerts, saveIncidents, saveIdeas, saveIngredients, saveProducts, saveMktData, saveCombos, saveClockRecords, saveChecklistRecords, saveAlbaranes, saveStockData, saveGamification, saveWeekTasks, savePriceHistory, savePrepSteps, saveStockMins, saveCateringLeads, saveCateringPresupuestos, saveCierresCaja, saveFraudeData, createProduct } from "./lib/db";

var dbModule = { loadAllData, savePromotions, saveStockAlerts, saveIncidents, saveIdeas, saveIngredients, saveProducts, saveMktData, saveCombos, saveClockRecords, saveChecklistRecords, saveAlbaranes, saveStockData, saveGamification, saveWeekTasks, savePriceHistory, savePrepSteps, saveStockMins, saveCateringLeads, saveCateringPresupuestos, saveCierresCaja, saveFraudeData, createProduct };
...
export default function App() {
  ...
  useEffect(function() {
    ...
    dbModule.loadAllData().then(function(data) {
      if (data) {
        ...
        if (data.stockMins) stockMins[1](data.stockMins);
        if (data.cateringLeads && data.cateringLeads.length > 0) cateringLeads[1](data.cateringLeads);
        if (data.cateringPresupuestos && data.cateringPresupuestos.length > 0) cateringPresupuestos[1](data.cateringPresupuestos);
        if (data.cierresCaja && data.cierresCaja.length > 0) cierresCaja[1](data.cierresCaja);
        if (data.fraudeData && data.fraudeData.length > 0) fraudeData[1](data.fraudeData);
  ...
  useEffect(function() {
    ...
    saveTimerRef.current = setTimeout(function() {
      ...
      Promise.resolve()
        ...
        .then(function() { if (dbModule.saveStockMins) return dbModule.saveStockMins(stockMins[0]).catch(function(){}); })
        .then(function() { if (dbModule.saveCateringLeads) return dbModule.saveCateringLeads(cateringLeads[0]).catch(function(){}); })
        .then(function() { if (dbModule.saveCateringPresupuestos) return dbModule.saveCateringPresupuestos(cateringPresupuestos[0]).catch(function(){}); })
        .then(function() { if (dbModule.saveCierresCaja) return dbModule.saveCierresCaja(cierresCaja[0]).catch(function(){}); })
        .then(function() { if (dbModule.saveFraudeData) return dbModule.saveFraudeData(fraudeData[0]).catch(function(){}); })
        .then(function() {
  ...
  }, [promosData[0], stockAlerts[0], incidents[0], ideasState[0], savedCombos[0], clockRecords[0], checklistRecords[0], albaranesData[0], stockData[0], gamification[0], weekTasks[0], stockMins[0], cateringLeads[0], cateringPresupuestos[0], cierresCaja[0], fraudeData[0]]);
  ...
  // Navigation refs (must be before early return)
  ...
  var allNav = [
    ...
    { k: "fichas-emp", l: "Recetas", roles: ["encargado"], group: "ops" },
  ...
  function navigateTo(pageKey) {
    if (pg[0] !== pageKey) pageHistory.current.push(pg[0]);
    if (pageHistory.current.length > 20) pageHistory.current = pageHistory.current.slice(-20);
    pg[1](pageKey);
    var targetArea = null;
  ...
  function goBack() {
    if (pageHistory.current.length > 0) {
      var prev = pageHistory.current.pop();
      pg[1](prev);
      var targetArea = null;
      for (var na = 0; na < allNav.length; na++) { if (allNav[na].k === prev) { targetArea = allNav[na].group; break; } }
      if (targetArea) openArea[1](targetArea);
    }
  }
  function handleTouchStart(e) {
    var touch = e.touches[0];
    if (touch.clientX < 25) touchStart.current = { x: touch.clientX, y: touch.clientY, time: Date.now() };
  ...
  function handleTouchEnd(e) {
    ...
    if (dx > 80 && dy < 100 && dt < 400 && pageHistory.current.length > 0) goBack();
  ...
  function handleBottomTabClick(tab) {
    if (tab.k.charAt(0) === "_") {
      if (tab.group === "mas" || tab.group === "ops") {
        activeBottomTab[1](activeBottomTab[0] === tab.group ? null : tab.group);
        return;
      }
      if (tab.group === "catering") {
        if (hasCatering) {
          activeBottomTab[1](null);
          navigateTo("catering-dash");
        } else {
          activeBottomTab[1](activeBottomTab[0] === "catering-blocked" ? null : "catering-blocked");
        }
        return;
      }
  ...
  return (
    <div style={{ fontFamily: "'Outfit', system-ui, sans-serif", background: "#f6f4f0", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      <div style={{ background: "#1a1a1a", padding: "10px 16px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div className="op-header-info" onClick={function() { if (role === "encargado" || role === "empleado") { navigateTo("mi-perfil"); activeBottomTab[1](null); } }} style={{ cursor: role === "encargado" || role === "empleado" ? "pointer" : "default" }}>
          ...
          <div className="op-header-text"><div style={{ fontSize: 12, fontWeight: 600, color: "#eee" }}>{usr[0].name}</div><div style={{ fontSize: 10, color: "#888" }}>{usr[0].role}{usr[0].local ? " - " + usr[0].local : ""}{role === "encargado" || role === "empleado" ? " | Perfil →" : ""}</div></div>
        </div>
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 48px)" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {isMobile[0] && activeBottomTab[0] === "ops" && (
            <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 68, background: "#111", zIndex: 99, overflowY: "auto", padding: "20px 16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <div style={{ fontSize: 18, fontWeight: 800, color: "#fff" }}>⚙️ Operaciones</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {(tabSubMenus.ops || []).map(function(n) {
          ...
          {/* CATERING BLOCKED OVERLAY */}
          {isMobile[0] && activeBottomTab[0] === "catering-blocked" && (
            <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 68, background: "#111", zIndex: 99, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", padding: "40px" }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>🔒</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: "#fff", marginBottom: 8, textAlign: "center" }}>Catering no disponible</div>
              <div style={{ fontSize: 13, color: "#888", textAlign: "center", maxWidth: 300, lineHeight: 1.6 }}>Tu usuario no tiene acceso al modulo de Catering. Contacta con Ale para que te lo habilite.</div>
              <button onClick={function() { activeBottomTab[1](null); }} style={{ marginTop: 20, background: "#333", color: "#aaa", border: "none", borderRadius: 8, padding: "8px 20px", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Cerrar</button>
            </div>
          )}

          {/* FULL-SCREEN "Mas" OVERLAY */}
          ...
          {/* PAGE CONTENT */}
          <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} style={{ padding: isMobile[0] ? "20px 14px 90px" : "24px 28px", maxWidth: 1400, margin: "0 auto", overflowX: "hidden" }}>
...
var USERS = [
  ...
  { user: "jesus", pass: "jesus2026", name: "Jesus", role: "encargado", local: "Los Remedios", catering: true },
...
function NovedadesBlock(props) {
  ...
  if (role !== "community") {
    ...
    var stockMinAlerts = 0;
    var sData = props.stockData ? props.stockData[0] || {} : {};
    var sMins = props.stockMins ? props.stockMins[0] || {} : {};
    var LOCS = ["San Luis","Los Remedios","Sevilla Este"];
    var userLoc = props.user ? props.user.local : null;
    var checkLocs = role === "socio" ? LOCS : (userLoc ? [userLoc] : LOCS);
    for (var li = 0; li < checkLocs.length; li++) {
      var ls = sData[checkLocs[li]] || {};
      for (var sk in ls) {
        var mk = sk + "_" + checkLocs[li];
        if (sMins[mk] > 0 && ls[sk].qty < sMins[mk]) stockMinAlerts++;
      }
    }
    if (stockMinAlerts > 0) items.push({ icon: "⚠️", text: stockMinAlerts + " ingrediente" + (stockMinAlerts > 1 ? "s" : "") + " bajo minimo de stock", color: "#DC2626", page: "albaranes", priority: 1 });
  }
...
function ProdView(props) {
  ...
  function saveNewProduct() {
    ...
    props.setProd(props.products.concat([newProduct]));
    // Save to Supabase via RPC
    if (dbModule && dbModule.createProduct) {
      dbModule.createProduct(
        { id: recId, name: "Esc " + np.name, notes: np.desc },
        np.ingredients.map(function(x) { return { refId: x.ingId, qty: x.qty }; }),
        { id: prodId, name: np.name, category: np.category, packQty: np.packQty || 1, weekSales: np.weekSales || 0 },
        np.prices
      );
    }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 13, color: "#888" }}>Guias de preparacion paso a paso | {fl.length} recetas disponibles</div>
      </div>
      <input value={ss[0]} onChange={function(e) { ss[1](e.target.value); }} placeholder="Buscar receta..." style={{ maxWidth: 320, padding: "10px 14px", border: "1.5px solid #e5e5e5", borderRadius: 10, fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: "inherit", marginBottom: 16 }} />
...
function CateringView(props) {
  ...
  var MODALIDADES = ["En Accion", "Grill & Go", "Experience", "Pack Grupos"];
  ...
  var PRECIOS = {
    ...
    "Pack Grupos": { "A+B+C": 17.90, "A+B": 15.90, "Solo A": 9.90 },
  };
  var EXTRAS_CATALOG = [
  ...
  function generatePresupuestoPDF() {
    ...
    var linkedLead = null;
    if (f.leadId) { for (var ll = 0; ll < leads[0].length; ll++) { if (leads[0][ll].id === f.leadId) { linkedLead = leads[0][ll]; break; } } }
    var clientName = f.clienteNombre || (linkedLead ? linkedLead.nombre : "Cliente");
    var clientEmail = f.clienteEmail || (linkedLead ? linkedLead.email : "");
    var clientTel = f.clienteTel || (linkedLead ? linkedLead.telefono : "");
    var today = new Date().toLocaleDateString("es-ES", { day: "2-digit", month: "long", year: "numeric" });
    var presuNum = "OP-" + new Date().getFullYear() + "-" + String(presupuestos[0].length + 1).padStart(3, "0");

    var extrasText = "";
    if (f.tipo !== "Pack Grupos" && f.extras && f.extras.length > 0) {
      for (var exi = 0; exi < f.extras.length; exi++) {
        for (var exc = 0; exc < EXTRAS_CATALOG.length; exc++) {
          if (EXTRAS_CATALOG[exc].id === f.extras[exi]) extrasText += "<tr><td style='padding:8px 12px;border-bottom:1px solid #eee'>" + EXTRAS_CATALOG[exc].name + "</td><td style='padding:8px 12px;border-bottom:1px solid #eee;text-align:right'>" + f.personas + "</td><td style='padding:8px 12px;border-bottom:1px solid #eee;text-align:right'>" + EXTRAS_CATALOG[exc].price.toFixed(2) + " EUR</td><td style='padding:8px 12px;border-bottom:1px solid #eee;text-align:right;font-weight:700'>" + (EXTRAS_CATALOG[exc].price * f.personas).toFixed(2) + " EUR</td></tr>";
        }
      }
    }
    ...
    var html = "<!DOCTYPE html><html><head><meta charset='utf-8'><title>Presupuesto " + presuNum + "</title><style>@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}body{font-family:Helvetica,Arial,sans-serif;color:#333;margin:0;padding:30px 40px;font-size:13px;line-height:1.5}*{box-sizing:border-box}</style></head><body>" +
    "<div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:30px;border-bottom:3px solid #B45309;padding-bottom:20px'>" +
      "<div><div style='font-size:28px;font-weight:800;color:#B45309;letter-spacing:1px'>ORALE PADRE</div><div style='font-size:12px;color:#888;margin-top:4px'>Catering &amp; Eventos</div></div>" +
      "<div style='text-align:right;font-size:11px;color:#888;line-height:1.6'>La baba hasta el suelo, S.L.<br>NIF: B56488323<br>Avenida del Olivo, 59<br>41927 Mairena del Aljarafe<br>Sevilla, Espana</div>" +
    "</div>" +
    "<div style='display:flex;justify-content:space-between;margin-bottom:30px'>" +
      "<div style='background:#f8f8f8;border-radius:8px;padding:16px;flex:1;margin-right:12px'><div style='font-size:10px;color:#888;font-weight:600;letter-spacing:1px;margin-bottom:8px'>PRESUPUESTO PARA</div><div style='font-size:16px;font-weight:700'>" + clientName + "</div>" + (clientEmail ? "<div style='font-size:12px;color:#666'>" + clientEmail + "</div>" : "") + (clientTel ? "<div style='font-size:12px;color:#666'>" + clientTel + "</div>" : "") + (f.eventoDesc ? "<div style='font-size:12px;color:#B45309;margin-top:6px'>" + f.eventoDesc + "</div>" : "") + "</div>" +
      "<div style='background:#f8f8f8;border-radius:8px;padding:16px;width:200px'><div style='font-size:10px;color:#888;font-weight:600;letter-spacing:1px;margin-bottom:8px'>DETALLES</div><div style='font-size:12px;margin-bottom:4px'><strong>N.:</strong> " + presuNum + "</div><div style='font-size:12px;margin-bottom:4px'><strong>Fecha:</strong> " + today + "</div>" + (f.eventoFecha ? "<div style='font-size:12px;margin-bottom:4px'><strong>Evento:</strong> " + f.eventoFecha + "</div>" : "") + "<div style='font-size:12px'><strong>Validez:</strong> " + (f.validez || "15 dias") + "</div></div>" +
    "</div>" +
    "<table style='width:100%;border-collapse:collapse;margin-bottom:20px'>" +
      "<thead><tr style='background:#1a1a1a;color:#fff'><th style='padding:10px 12px;text-align:left;font-size:11px;font-weight:600'>CONCEPTO</th><th style='padding:10px 12px;text-align:right;font-size:11px;font-weight:600'>CANT.</th><th style='padding:10px 12px;text-align:right;font-size:11px;font-weight:600'>PRECIO/UD</th><th style='padding:10px 12px;text-align:right;font-size:11px;font-weight:600'>TOTAL</th></tr></thead>" +
      "<tbody>" +
        "<tr><td style='padding:10px 12px;border-bottom:1px solid #eee'><strong>" + f.tipo + "</strong> — " + f.producto + "<br><span style='font-size:11px;color:#888'>" + packDesc + "</span></td><td style='padding:10px 12px;border-bottom:1px solid #eee;text-align:right'>" + f.personas + " pers.</td><td style='padding:10px 12px;border-bottom:1px solid #eee;text-align:right'>" + calc.base.toFixed(2) + " EUR</td><td style='padding:10px 12px;border-bottom:1px solid #eee;text-align:right;font-weight:700'>" + (calc.base * f.personas).toFixed(2) + " EUR</td></tr>" +
        extrasText + transporteText +
      "</tbody>" +
    "</table>" +
    "<div style='display:flex;justify-content:flex-end;margin-bottom:30px'>" +
      "<div style='width:250px'>" +
        "<div style='display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;font-size:13px'><span>Subtotal</span><span>" + calc.subtotal.toFixed(2) + " EUR</span></div>" +
        "<div style='display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;font-size:13px'><span>IVA (10%)</span><span>" + calc.iva.toFixed(2) + " EUR</span></div>" +
        "<div style='display:flex;justify-content:space-between;padding:10px 0;font-size:18px;font-weight:800;color:#B45309'><span>TOTAL</span><span>" + calc.total.toFixed(2) + " EUR</span></div>" +
      "</div>" +
    "</div>" +
    (f.tipo === "Pack Grupos" ? "<div style='background:#FFF7ED;border-radius:8px;padding:16px;margin-bottom:20px;border-left:3px solid #B45309'><div style='font-size:12px;font-weight:700;color:#B45309;margin-bottom:8px'>OPCIONES INCLUIDAS EN EL PACK</div><div style='font-size:11px;color:#666;line-height:1.8'>" + (f.producto === "A+B+C" || f.producto === "A+B" || f.producto === "Solo A" ? "<strong>Opcion A (Burrito/Desnudo):</strong> Lady Cochinita, Dona Dolores, Black Chancho, Pollo Pastor, Don Hampi, Don Juarez, Pollo Padre | Desnudo Juarez, Desnudo Cochinita, Desnudo Black Chancho, Desnudo Dolores<br>" : "") + (f.producto === "A+B+C" || f.producto === "A+B" ? "<strong>Opcion B (Tacos):</strong> Taco Pollo Pastor, Taco Lady Cochinita, Tacos Chancho<br>" : "") + (f.producto === "A+B+C" ? "<strong>Opcion C (Postres):</strong> Filipinos Blancos, Dulce de Leche, Kinder Bueno, Galleta Lotus" : "") + "</div></div>" : "") +
    (f.notas ? "<div style='background:#f8f8f8;border-radius:8px;padding:14px;margin-bottom:20px'><div style='font-size:11px;font-weight:700;color:#888;margin-bottom:4px'>NOTAS</div><div style='font-size:12px;color:#555'>" + f.notas + "</div></div>" : "") +
    "<div style='background:#f8f8f8;border-radius:8px;padding:14px;margin-bottom:20px'><div style='font-size:11px;font-weight:700;color:#888;margin-bottom:6px'>CONDICIONES</div><div style='font-size:11px;color:#666;line-height:1.8'>- Precio incluye entrega dentro de Sevilla capital y alrededores<br>" + (f.tipo === "Pack Grupos" ? "- Minimo 25 packs<br>" : "") + "- Se pueden elaborar propuestas sin gluten y veganas bajo peticion<br>- Se pueden modificar ingredientes<br>- Pago: 50% al confirmar, 50% el dia del evento<br>- Cancelacion: Hasta 48h antes sin coste. Posterior, 50% del presupuesto</div></div>" +
    "<div style='text-align:center;padding-top:20px;border-top:2px solid #B45309;font-size:11px;color:#888'>ORALE PADRE — www.oralepadre.com — hola@oralepadre.com — Sevilla</div>" +
    "</body></html>";
      ...
      {tab[0] === "presupuestos" && (
        <div>
          ...
          {showPresu[0] && (function() {
            var isPack = presuForm[0].tipo === "Pack Grupos";
            var prodOptions = isPack ? ["A+B+C", "A+B", "Solo A"] : ["Completo", "Mitades", "Pack"];
            // Auto-set product when changing type
            if (isPack && ["Completo", "Mitades", "Pack"].indexOf(presuForm[0].producto) >= 0) {
              presuForm[1](Object.assign({}, presuForm[0], { producto: "A+B+C" }));
            }
            if (!isPack && ["A+B+C", "A+B", "Solo A"].indexOf(presuForm[0].producto) >= 0) {
              presuForm[1](Object.assign({}, presuForm[0], { producto: "Completo" }));
            }
            var calc = calcPresupuesto();
            return (
              <div style={{ ...crd, marginBottom: 16, borderLeft: "4px solid #B45309" }}>
                <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 16 }}>Generador de Presupuesto</div>

                {/* Client info */}
                <div style={{ fontSize: 12, fontWeight: 700, color: "#B45309", marginBottom: 8 }}>DATOS DEL CLIENTE</div>
                <div style={{ display: "grid", gridTemplateColumns: props.isMobile ? "1fr" : "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
                  <div><div style={{ fontSize: 11, fontWeight: 600, color: "#888", marginBottom: 4 }}>NOMBRE / EMPRESA</div><input value={presuForm[0].clienteNombre} onChange={function(e) { presuForm[1](Object.assign({}, presuForm[0], { clienteNombre: e.target.value })); }} placeholder="Empresa o persona" style={inp} /></div>
                  <div><div style={{ fontSize: 11, fontWeight: 600, color: "#888", marginBottom: 4 }}>EMAIL</div><input value={presuForm[0].clienteEmail} onChange={function(e) { presuForm[1](Object.assign({}, presuForm[0], { clienteEmail: e.target.value })); }} placeholder="email@empresa.com" style={inp} /></div>
                  <div><div style={{ fontSize: 11, fontWeight: 600, color: "#888", marginBottom: 4 }}>TELEFONO</div><input value={presuForm[0].clienteTel} onChange={function(e) { presuForm[1](Object.assign({}, presuForm[0], { clienteTel: e.target.value })); }} placeholder="654 000 000" style={inp} /></div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: props.isMobile ? "1fr" : "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
                  <div><div style={{ fontSize: 11, fontWeight: 600, color: "#888", marginBottom: 4 }}>FECHA EVENTO</div><input type="date" value={presuForm[0].eventoFecha} onChange={function(e) { presuForm[1](Object.assign({}, presuForm[0], { eventoFecha: e.target.value })); }} style={inp} /></div>
                  <div><div style={{ fontSize: 11, fontWeight: 600, color: "#888", marginBottom: 4 }}>DESCRIPCION EVENTO</div><input value={presuForm[0].eventoDesc} onChange={function(e) { presuForm[1](Object.assign({}, presuForm[0], { eventoDesc: e.target.value })); }} placeholder="Boda, corporativo..." style={inp} /></div>
                  <div><div style={{ fontSize: 11, fontWeight: 600, color: "#888", marginBottom: 4 }}>VINCULAR A LEAD</div><select value={presuForm[0].leadId} onChange={function(e) { var lid = e.target.value; var ll = null; for (var i = 0; i < leads[0].length; i++) { if (leads[0][i].id === lid) { ll = leads[0][i]; break; } } presuForm[1](Object.assign({}, presuForm[0], { leadId: lid, clienteNombre: ll ? ll.nombre : presuForm[0].clienteNombre, clienteEmail: ll ? ll.email : presuForm[0].clienteEmail, clienteTel: ll ? ll.telefono : presuForm[0].clienteTel })); }} style={sel}><option value="">Sin vincular</option>{leads[0].filter(function(l) { return l.estado !== "Perdido"; }).map(function(l) { return <option key={l.id} value={l.id}>{l.nombre}</option>; })}</select></div>
                </div>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {EXTRAS_CATALOG.map(function(ex) {
                        ...
                        }} style={{ padding: "6px 14px", borderRadius: 8, border: isOn ? "2px solid #B45309" : "1px solid #e5e5e5", background: isOn ? "#B4530908" : "#fff", color: isOn ? "#B45309" : "#888", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>{ex.name} (+{ex.price}EUR/pers)</button>;
                      })}
                    </div>
                  </div>
                )}
                ...
                <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
                  <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 600 }}>Transporte extra (+{TRANSPORTE_PRICE}EUR)</span>
                  </label>
                </div>
                <div style={{ marginBottom: 16 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#888", marginBottom: 4 }}>NOTAS PARA EL PRESUPUESTO</div><input value={presuForm[0].notas} onChange={function(e) { presuForm[1](Object.assign({}, presuForm[0], { notas: e.target.value })); }} placeholder="Notas adicionales..." style={inp} /></div>

                {/* Result */}
                <div style={{ background: "#111", borderRadius: 12, padding: "20px", color: "#fff", marginBottom: 16 }}>
                  <div style={{ display: "grid", gridTemplateColumns: isSocio ? "1fr 1fr 1fr 1fr" : "1fr 1fr 1fr", gap: 16, textAlign: "center" }}>
                    <div><div style={{ fontSize: 10, color: "#888" }}>SUBTOTAL</div><div style={{ fontSize: 20, fontWeight: 800 }}>{calc.subtotal.toFixed(0)}EUR</div></div>
                    <div><div style={{ fontSize: 10, color: "#888" }}>IVA (10%)</div><div style={{ fontSize: 20, fontWeight: 800 }}>{calc.iva.toFixed(0)}EUR</div></div>
                    <div><div style={{ fontSize: 10, color: "#888" }}>TOTAL</div><div style={{ fontSize: 24, fontWeight: 800, color: "#4ADE80" }}>{calc.total.toFixed(2)}EUR</div></div>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button onClick={function() { savePresupuesto(); }} style={{ padding: "10px 20px", borderRadius: 10, background: "#047857", color: "#fff", border: "none", fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>💾 Guardar</button>
                  <button onClick={generatePresupuestoPDF} style={{ padding: "10px 20px", borderRadius: 10, background: "#B45309", color: "#fff", border: "none", fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>📄 Generar PDF</button>
                  <button onClick={function() { var calc2 = calcPresupuesto(); var txt = "Presupuesto Orale Padre Catering\n" + presuForm[0].tipo + " — " + presuForm[0].producto + "\n" + presuForm[0].personas + " personas\nTotal: " + calc2.total.toFixed(2) + "EUR (IVA incl.)\n\nContacto: hola@oralepadre.com"; window.open("https://wa.me/?text=" + encodeURIComponent(txt)); }} style={{ padding: "10px 20px", borderRadius: 10, background: "#25D366", color: "#fff", border: "none", fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>💬 WhatsApp</button>
                  <button onClick={function() { var calc2 = calcPresupuesto(); var subj = "Presupuesto Orale Padre — " + presuForm[0].tipo; var body = "Hola " + (presuForm[0].clienteNombre || "") + ",\n\nAdjunto presupuesto para " + presuForm[0].personas + " personas.\n\nTipo: " + presuForm[0].tipo + " — " + presuForm[0].producto + "\nTotal: " + calc2.total.toFixed(2) + "EUR (IVA incl.)\n\nQuedamos a tu disposicion.\n\nOrale Padre\nhola@oralepadre.com"; window.open("mailto:" + (presuForm[0].clienteEmail || "") + "?subject=" + encodeURIComponent(subj) + "&body=" + encodeURIComponent(body)); }} style={{ padding: "10px 20px", borderRadius: 10, background: "#1E40AF", color: "#fff", border: "none", fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>📧 Email</button>
                </div>
              </div>
            );
          })()}
      ...
      {mainTab[0] === "stock" && (
        <div>
          ...
          {(function() {
            ...
            var mins = props.stockMins ? props.stockMins[0] : {};
            var loc = stockViewLocal[0];
            var lowItems = [];
            for (var k in localSt) {
              var minKey = k + "_" + loc;
              var minQty = mins[minKey] || 0;
              if (minQty > 0 && localSt[k].qty < minQty) lowItems.push({ nombre: localSt[k].nombre, qty: localSt[k].qty, min: minQty, unit: localSt[k].unit });
            }
            if (lowItems.length === 0) return null;
            return (
              <div style={{ ...crd, padding: 14, marginBottom: 12, borderLeft: "4px solid #DC2626", background: "#FEF2F210" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#DC2626", marginBottom: 6 }}>⚠️ {lowItems.length} ingrediente{lowItems.length > 1 ? "s" : ""} bajo minimo en {loc}</div>
                {lowItems.map(function(li) {
                  return <div key={li.nombre} style={{ fontSize: 12, color: "#555", marginBottom: 3 }}>
                    <strong>{li.nombre}</strong>: {li.qty} {li.unit} (minimo: {li.min} {li.unit}) — <span style={{ color: "#DC2626", fontWeight: 700 }}>PEDIR</span>
                  </div>;
                })}
              </div>
            );
          ...
          {(function() {
            var localSt = stock[stockViewLocal[0]] || {};
            var mins = props.stockMins ? props.stockMins[0] : {};
            var setMins = props.stockMins ? props.stockMins[1] : function(){};
            var loc = stockViewLocal[0];
            var items = [];
            ...
            return (
              <div style={{ ...crd, overflowX: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <div style={{ fontSize: 14, fontWeight: 700 }}>Stock actual — {stockViewLocal[0]} ({items.length} ingredientes)</div>
                  <div style={{ fontSize: 11, color: "#888" }}>Edita "Min" para activar alertas</div>
                </div>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead><tr style={{ borderBottom: "2px solid #eee" }}>
                    ...
                    <th style={{ textAlign: "center", padding: "8px 6px", color: "#888", fontWeight: 600, fontSize: 11 }}>Ud.</th>
                    <th style={{ textAlign: "right", padding: "8px 6px", color: "#888", fontWeight: 600, fontSize: 11, minWidth: 70 }}>Min</th>
                    <th style={{ textAlign: "center", padding: "8px 6px", color: "#888", fontWeight: 600, fontSize: 11 }}>Estado</th>
                    <th style={{ textAlign: "right", padding: "8px 6px", color: "#888", fontWeight: 600, fontSize: 11 }}>Ultima entrada</th>
                  </tr></thead>
                  <tbody>
                    {items.map(function(it) {
                      var minKey = it.id + "_" + loc;
                      var minQty = mins[minKey] || 0;
                      var isBelow = minQty > 0 && it.qty < minQty;
                      var isOut = it.qty <= 0;
                      var bgColor = isOut ? "#FEF2F2" : isBelow ? "#FFFBEB" : "#fff";
                      return (
                        <tr key={it.id} style={{ borderBottom: "1px solid #f5f5f5", background: bgColor }}>
                          <td style={{ padding: "8px 6px", fontWeight: 600 }}>
                            {it.nombre}
                          </td>
                          <td style={{ padding: "8px 6px", textAlign: "right", fontSize: 15, fontWeight: 800, color: isOut ? "#DC2626" : isBelow ? "#D97706" : "#047857" }}>{it.qty}</td>
                          <td style={{ padding: "8px 6px", textAlign: "center", color: "#888" }}>{it.unit}</td>
                          <td style={{ padding: "8px 6px", textAlign: "right" }}>
                            <input type="number" value={minQty || ""} placeholder="—" onChange={function(e) {
                              var newMins = Object.assign({}, mins);
                              newMins[it.id + "_" + loc] = parseFloat(e.target.value) || 0;
                              setMins(newMins);
                            }} style={{ width: 55, padding: "3px 5px", border: "1px solid #e5e5e5", borderRadius: 6, fontSize: 12, textAlign: "right", fontFamily: "inherit", background: minQty > 0 ? "#F0FDF4" : "#fff" }} />
                          </td>
                          <td style={{ padding: "8px 6px", textAlign: "center" }}>
                            {isOut ? <span style={{ fontSize: 10, fontWeight: 700, color: "#DC2626", padding: "2px 6px", borderRadius: 6, background: "#FEE2E2" }}>AGOTADO</span> :
                             isBelow ? <span style={{ fontSize: 10, fontWeight: 700, color: "#D97706", padding: "2px 6px", borderRadius: 6, background: "#FEF3C7" }}>BAJO</span> :
                             minQty > 0 ? <span style={{ fontSize: 10, fontWeight: 700, color: "#047857", padding: "2px 6px", borderRadius: 6, background: "#ECFDF5" }}>OK</span> :
